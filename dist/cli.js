...
class PythonRunner {
    ...
    run(rawArgs) {
        ...
        const proc = (0, child_process_1.spawn)(this.pythonPath, pyArgs, { stdio: "inherit" });
    ...
    buildPythonCommand(args, paths) {
        ...
        if (args.device)
            pyArgs.push("--device", args.device);
        if (args.height !== undefined)
            pyArgs.push("--height", String(args.height));
        if (args.fps !== undefined)
            pyArgs.push("--fps", String(args.fps));
        if (args.margin !== undefined)
            pyArgs.push("--margin", String(args.margin));
        if (args.smooth !== undefined)
            pyArgs.push("--smooth", String(args.smooth));
        ...
        if (args.conf !== undefined)
            pyArgs.push("--conf", String(args.conf));
        if (args.bootstrapFrames !== undefined)
            pyArgs.push("--bootstrap-frames", String(args.bootstrapFrames));
        if (args.roiSize !== undefined)
            pyArgs.push("--roi", String(args.roiSize));
        if (args.tracker)
            pyArgs.push("--tracker", args.tracker);
        if (args.backend)
            pyArgs.push("--backend", args.backend);
        if (args.targetClass)
            pyArgs.push("--target-class", args.targetClass);
        if (args.noAppearance)
            pyArgs.push("--no-appearance");
        if (args.profile)
            pyArgs.push("--profile");
        if (args.noTtaRecovery)
            pyArgs.push("--no-tta-recovery");
        if (args.imgsz !== undefined)
            pyArgs.push("--imgsz", String(args.imgsz));
        if (args.useFfmpeg)
            pyArgs.push("--use-ffmpeg");
        if (args.detInterval !== undefined)
            pyArgs.push("--det-interval", String(args.detInterval));
        if (args.jerk !== undefined)
            pyArgs.push("--jerk", String(args.jerk));
        if (args.strictCenter)
            pyArgs.push("--strict-center");
        if (args.stickyWindow)
            pyArgs.push("--sticky-window");
        if (args.centerBound !== undefined)
            pyArgs.push("--center-bound", String(args.centerBound));
        if (args.detectEveryFrame)
            pyArgs.push("--detect-every-frame");
        if (args.fullDetect)
            pyArgs.push("--full-detect");
        if (args.threePass)
